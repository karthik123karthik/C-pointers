...
- printf("%d", *ptr); it will print only one byte of data. as size of char is one byte.
  ...
  - Ok first understand what is compiler.
  - compiler is a program that will convert source code (human readable code) to binary code (machine code).
 
  - COMPILATION PROCESS
  - .c -> preprocessor -> .i -> compilation -> .s -> assembler -> .o -> linker -> .out
